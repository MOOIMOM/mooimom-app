...
export function* postGetAllEventForm(api, action) {
  const { data } = action
  // get current data from Store
  // const currentData = yield select(RegisterSelectors.getData)
  // make the call to the api
  const response = yield call(api.postGetAllEventForm, data)
  // success?
  if (response.data.success === 1) {
    ...
    yield put(GetAllEventFormActions.getAllEventFormSuccess(response.data))
    ...
    yield put(GetAllEventFormActions.getAllEventFormFailure(err))
  }
