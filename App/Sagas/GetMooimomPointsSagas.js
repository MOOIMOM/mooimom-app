...
export function* postGetMooimomPoints(api, action) {
  const { data } = action
  // get current data from Store
  // const currentData = yield select(RegisterSelectors.getData)
  // make the call to the api
  const response = yield call(api.postGetMooimomPoints, data)
  // success?
  if (response.data.success === 1) {
    ...
    yield put(GetMooimomPointsActions.getMooimomPointsSuccess(response.data))
    ...
    yield put(GetMooimomPointsActions.getMooimomPointsFailure(response.data))
  }
}
