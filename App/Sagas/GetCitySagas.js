...
export function* postGetCity(api, action) {
  const { data } = action
  // get current data from Store
  // const currentData = yield select(RegisterSelectors.getData)
  // make the call to the api
  const response = yield call(api.postGetCity, data)
  // success?
  if (response.data.success === 1) {
    ...
    yield put(CityActions.getCityFailure(err))
  }
    ...
    yield put(CityActions.getCityFailure(response.data))
  }
}
