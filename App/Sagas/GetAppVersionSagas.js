...
export function* postGetAppVersion(api, action) {
  const { data } = action
  // get current data from Store
  // const currentData = yield select(RegisterSelectors.getData)
  // make the call to the api
  const response = yield call(api.postGetAppVersion, data)
  // success?
  if (response.data.success === 1) {
    ...
    yield put(GetAppVersionActions.getAppVersionSuccess(response.data))
    ...
    yield put(GetAppVersionActions.getAppVersionFailure(err))
  }
