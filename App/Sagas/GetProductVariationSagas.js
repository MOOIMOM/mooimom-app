...
export function* postGetProductVariation(api, action) {
  const { data } = action
  // get current data from Store
  // const currentData = yield select(RegisterSelectors.getData)
  // make the call to the api
  const response = yield call(api.postGetProductVariation, data)
  // success?
  if (response.data.success === 1) {
    ...
    console.log(data)
    yield put(GetProductActions.getProductVariationSuccess(response.data))
