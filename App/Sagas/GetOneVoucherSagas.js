...
export function* postGetOneVoucher(api, action) {
  const { data } = action
  // get current data from Store
  // const currentData = yield select(RegisterSelectors.getData)
  // make the call to the api
  const response = yield call(api.postGetOneVoucher, data)
  // success?
  if (response.data.success === 1) {
    if (__DEV__) console.tron.log(response)
    // You might need to change the response here - do this with a 'transform',
    // located in ../Transforms/. Otherwise, just pass the data back from the api.
    yield put(GetOneVoucherActions.getOneVoucherSuccess(response.data))
    ...
    yield put(GetOneVoucherActions.getOneVoucherSuccess(err))
  }
