...
export function * postGetAddress(api, action) {
  ...
  const response = yield call(api.postGetAddress, data)
    ...
    if (response.data.success === 1) {
      ...
      yield put(AddressActions.getAddressSuccess(response.data))
      ...
      yield put(AddressActions.getAddressFailure(err))
    } else {
      yield put(AddressActions.getAddressFailure(response.data))
    }
}
