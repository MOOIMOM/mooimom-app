...
export default class NewProductCard extends Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  render() {
    if (!this.state.product.sales_price) return (<View />)
    ...
    let price = convertToRupiah(this.state.product.sales_price)
    let disc = ""
    ...
    if (this.state.product.image && this.state.product.image !== '') { image = { uri: this.state.product.image } }
    return (
      <View style={styles.item}>
        ...
        <View style={styles.productNameContainer}>
          <Text numberOfLines={2} ellipsizeMode={"tail"} style={styles.name}>{this.state.product.name}</Text>
        </View>
